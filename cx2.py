...
def login():
    ...
    try:
        ...
        list_c = get_courses_url(s, browser)  # 列出课程用于选择要进行筛选的课程
    except Exception:
        tk.messagebox.showerror(title='运行时出现错误', message='\n可能是账号密码验证码输入错误，请关闭后重新打开进行登录')
...
def count_etimes():
    ...
    try:
        ...
        if err_grp != '':
            tk.messagebox.showwarning(title='获取一些小组时出现问题', message='以下小组的帖子获取数据失败，重试可能会解决问题：\n' + err_grp)

        # 输出统计数据
        export_count(dic_grp)

    except Exception as e:
        tk.messagebox.showerror(title='运行时出现错误', message=str(e))
    finally:
        # 关闭浏览器
...
wait_class = tk.Label(window_class, text='正在执行操作，请稍等...')
# 创建滚动条
sb1 = tk.Scrollbar(window_class)
sb1.pack(side='right', fill='y')
# 创建 listbox
class_list = tk.Listbox(window_class, width=43, height=10, font=('Arial', 14), yscrollcommand=sb1.set)
